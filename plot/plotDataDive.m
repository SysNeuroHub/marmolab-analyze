...
cuedind = d.cueOn & d.complete; % & d.targetloc == d.theta;
uncuedind = ~d.cueOn & d.complete; % & d.targetloc == d.theta;

cuedSp = trialSpike(d,'onset','sTarget','trind', cuedind, 'bn',[-300,500]);
uncuedSp = trialSpike(d,'onset','sTarget','trind', uncuedind, 'bn',[-300,500]);
...
spikephase = trialSpikeLFPPhase(d,'onset','sTarget','trind', d.complete, 'bn',[-500,500],'method','GP','fk',[5, 40]);
...
spikephase = trialSpikeLFPPhase(d,'onset','sTarget','trind', d.complete, 'bn',[-500,500],'method','MT','tapers',[0.5,10],'fk',[4 12 20 40]);
spikephase = squeeze(spikephase);
